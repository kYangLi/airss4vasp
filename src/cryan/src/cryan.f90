...
!!$    character(len=10) :: cform
  ...
  subroutine get_arguments()
    ...
    integer :: iargc,num_args,na,task_index
...
121       usage=.true.
    ...
    ! Move the 'rank' to the end of the task list, since the task behead it 
    !   will be ignored... 
    do task_index=1,num_tasks
        if (task(task_index)=='rank') then
            task(task_index)=task(num_tasks)
            task(num_tasks)='rank'
        end if
    end do
